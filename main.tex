...
\usepackage{booktabs}
\usepackage{pgfplots}
\title[Learning Abstract Planning Domains]{Learning abstract planning domains and mappings to real world perceptions
\subtitle{\only<2>{{\color {red} (Where am I? Let me ask to the real world!)}}
}
}
% \date{\today}
\date{}
...
\begin{axis}[every axis plot post/.append style={
  ...
  \addplot {gauss(2,1.5)} node [pos=.25,above] {$f(x,s_1)$};
  \addplot {gauss(10,1.5)} node [pos=.7 5,above] {$f(x,s_2)$};;
  \addplot {.1} node[pos=0.7,above] {$(1-\epsilon)\cdot f_\max$};
  \only<2>{\node at (1,0) {$\bullet$};}
  \only<2>{\node[blue] at (1,0.21) {$\bullet$};}  
...
\item the world is a $5\times5$ building with random walls
\item initial model with $S=\{s_0,s_1,s_2,s_3\}$ actions $A=\{n,s,e,w\}$;
\item Performances depending on $\alpha=0.5$, $\beta=0.5$, $\epsilon=0.5$
\item the agent has to achieve 10 random goals
\end{itemize}
