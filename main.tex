...
\begin{enumerate}
\item Pianifica (e se non trovi il piano esplora) 
\item esegui un'azione e osserva i sensori ($\bx$)
\item \alert<2>{decidi in quale stato sei in base a max likelihood ($\argmax_{s\in S}f(\bx,s)$)}
\item \alert<2>{se nessuno stato giustifica sufficientemente l'osservaizione $\bx$ introduci un nuovo stato $s_{new}$}
\item aggiorna $\gamma$
\item aggiorna $f$ 
\end{enumerate}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item In order to estimate the quality of the model generated by the \PAL\
...
\item We introduce a measure called \emph{divergence}. 
